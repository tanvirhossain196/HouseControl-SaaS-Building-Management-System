...
import { handleZinipayRent } from '@/services/rent-zinipay.service'

export const runtime = 'nodejs'
export const dynamic = 'force-dynamic'

/**
 * ZiniPay's callback for a rent payment.
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await handleZinipayRent({ invoiceId })
    console.info('[zinipay-rent]', invoiceId, result.outcome)

    return NextResponse.json(result, { status: 200 })
  } catch (error) {
    console.error('[zinipay-rent]', error)
